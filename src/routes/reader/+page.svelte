...
<div class="flex h-screen flex-col bg-background font-sans">
				...
				>
					...
					<div class="h-full w-full">
						{#if chapters[currentChapter]?.isCover}
							<div 
								class="h-full w-full bg-background transition-opacity duration-300"
	...
	<footer class="border-t border-border bg-background px-6 py-3 text-center">
		<p class="text-sm text-muted-foreground">
			...
			{#if chapters[currentChapter] && !chapters[currentChapter].isFrontmatter}
				• Chapter {chapters.slice(0, currentChapter + 1).filter(c => !c.isFrontmatter).length} of {chapters.filter(c => !c.isFrontmatter).length}
			{/if}
		</p>
	</footer>
</div>
...
<style>
	...
	:global(.prose .x-ebookmaker-coverpage), :global(.prose .x-ebookmaker-cover) { height: 100%; min-height: calc(100vh - 120px); display: flex; align-items: center; justify-content: center; padding: 0 !important; margin: 0 !important; }
